...
plot_resid.lm <- function(x, delog = F, ncol = NULL, nrow = NULL, title = NULL) {
  ...
  if (delog == T){
    #resid x fitted
    Presid_fit <- ggplot2::ggplot(x, ggplot2::aes(x = exp(.fitted), y = (((exp(ggplot2::fortify(x)[,1]) - exp(.fitted))*100)/ exp(ggplot2::fortify(x)[,1])))) +
      ggplot2::geom_point(alpha = 0.2) +
      ggplot2::geom_hline(yintercept = 0, size = 0.7) +
      ggplot2::labs(x = "fitted", y = "resid%") +
      ggplot2::ylim(-1*max(abs((((exp(ggplot2::fortify(x)[,1]) - exp(ggplot2::fortify(x)[,".fitted"]))*100) / exp(ggplot2::fortify(x)[,1])))),
                    max(abs(((exp(ggplot2::fortify(x)[,1]) - exp(ggplot2::fortify(x)[,".fitted"]))*100)/ exp(ggplot2::fortify(x)[,1])))) +
      ggplot2::theme_minimal()
    lstResidP[["Presid_fit"]] <- Presid_fit

    #resid x observed
    Presid_obs <- ggplot2::ggplot(x, ggplot2::aes(x = exp(ggplot2::fortify(x)[,1]), y = (((exp(ggplot2::fortify(x)[,1]) - exp(.fitted))*100)/ exp(ggplot2::fortify(x)[,1])))) +
      ggplot2::geom_point(alpha = 0.2) +
      ggplot2::geom_hline(yintercept = 0, size = 0.7) +
