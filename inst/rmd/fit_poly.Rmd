...
if (any(!is.null(dcom) & dcom == 0)){
  ...
  cubcom <- cubcom %>% 
    ...
    tidyr::fill(idade, especie, matgen, regime, htoco, dap, h, cdap, .direction = "updown") %>%
    dplyr::arrange(rf, talhao, ciclo, rotacao, arvore, dt_med, coleta, obs, desc(dcc)) %>% 
...
ggplot2::ggplot(data = pcub, ggplot2::aes(x = hih, y = did, color = casca)) +
  ...
  ggplot2::theme_minimal(base_size = 16) %+replace% ggplot2::theme(legend.title=element_blank(),
...
stats_vol <- db_poly_vol_cc %>% 
  dplyr::group_by(cdap) %>% 
  dplyr::summarise(sigma = sqrt(sum(.resid^2)/(n()-6)),
